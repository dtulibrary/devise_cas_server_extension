...
module Devise
  module CasServer
    module SessionsControllerBehaviour
      def self.included(target)
        target.send(:include, InstanceMethods)
        target.extend ClassMethods

        target.send(:prepend_before_filter, :renew_session, :only => [:new])
        target.send(:before_filter, :handle_cas_login_ticket, :only => [:new])
        target.send(:before_filter, :verify_cas_login_ticket, :only => [:create])
      end
      ...
      module InstanceMethods
        ...
        def after_sign_in_path_for(resource)
          ...
          if session['cas_server_service'].blank?
            logger.info("Successfully authenticated user '#{tgt.username}' at "+
              "'#{tgt.client_hostname}'. No service param was given, so we will "+
              "not redirect.")
          else
        ...
        def create_service_url(tgt)
          ...
          begin
            ...
            logger.error("The service '#{@session['cas_server_service']}' is not a "+
              "valid URI!")
        ...
        def handle_cas_login_ticket
          clean_url = Devise::Models::ServiceTicket.clean_service_url(
            params['service'])
          ...
          if session['cas_server_service']
            if tr && tr.granting_ticket
              ...
              logger.info("User '#{tr.granting_ticket.username}' authenticated "+
                "based on ticket granting cookie. Redirecting to service "+
                "'#{session['cas_server_service']}'.")
              # response code 303 means "See Other"
              # (see Appendix B in CAS Protocol spec)
              redirect_to url, :status => 303 if url
              logger.error("The service '#{session['server_cas_service']}' is not "+
                "a valid URI!")
              set_flash_message(:alert, 'cas_server.invalid_target_service')
            elsif @gateway
              logger.info("Redirecting unauthenticated gateway request to service "+
                "'#{session['cas_server_service']}'.")
              redirect_to session['cas_server_service'], :status => 303
            else
              logger.info("Proceeding with CAS login for service "+
                "#{session['cas_server_service'].inspect}.")
            end
          elsif @gateway
            logger.error("This is a gateway request but no service parameter "+
              "was given!")
        ...
        def send_logout_notification_for_service_ticket(st)
          ...
          req.set_form_data('logoutRequest' => %{<samlp:LogoutRequest
 xmlns:samlp="urn:oasis:names:tc:SAML:2.0:protocol"
 xmlns:saml="urn:oasis:names:tc:SAML:2.0:assertion" ID="#{rand}"
 Version="2.0" IssueInstant="#{time.rfc2822}">
          ...
          begin
            ...
            http.start do |conn|
              ...
              if response.kind_of? Net::HTTPSuccess
                logger.info "Logout notification successfully posted to "+
                  "#{st.service.inspect}."
                return true
              else
                logger.error "Service #{st.service.inspect} responed to logout "+
                  "notification with code '#{response.code}'!"
                return false
              end
            end
          rescue StandardError => e
            logger.error "Failed to send logout notification to service "+
              "#{st.service.inspect} due to #{e}"
            return false
          end
        end
